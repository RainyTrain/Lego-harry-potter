...
const ShipingForm = () => {
  const state = useTypedSelector((state) => state.figReducer.isFormValid);
  useEffect(() => {
    console.log(state);
  });
  ...
  return (
    <div className="container">
      ...
      <Formik
        ...
        onSubmit={(values) => {
          // const { name, surname, phone, email, date, adress, city, state, zip } = values;
          // dispatch(setValid(true));
          // dispatch(setData({ name, surname, phone, email, date, adress, city, state, zip }));
        }}>
        {(formik) => (
          <form
